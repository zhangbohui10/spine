...
public class AndroidGraphics extends AbstractGraphics implements Renderer, GLTextureView.Renderer {
	...
	WeakReference<AndroidApplicationBase> app;
	GL20 gl20;
	GL30 gl30;
	...
	public AndroidGraphics (AndroidApplicationBase application, AndroidApplicationConfiguration config,
		...
		this.config = config;
		this.app = new WeakReference<AndroidApplicationBase>(application);
	...
	protected void updatePpi () {
		...
		if (app.get() != null) {
			app.get().getWindowManager().getDefaultDisplay().getMetrics(metrics);
		}else {
			return;
		}
	...
	public void onSurfaceChanged (GL10 gl, int width, int height) {
		...
		if (created == false) {
			if (app.get() != null && app.get().getApplicationListener() != null) {
				app.get().getApplicationListener().create();
			}else {
				return;
			}
			created = true;
		...
		if (app.get() != null && app.get().getApplicationListener() != null) {
			app.get().getApplicationListener().resize(width, height);
		}
	}
	...
	public void onSurfaceCreated (GL10 gl, EGLConfig config) {
		...
		AndroidApplicationBase app = null;
	...
	public void onDrawFrame (GL10 gl) {
		...
		if (lresume) {
			...
			if (app != null && app.getApplicationListener() != null) {
