...
public class DefaultAndroidInput extends AbstractInput implements AndroidInput {
	...
	final WeakReference<Application> app;
	...
	protected Orientation nativeOrientation = Orientation.Landscape;
	...
	public DefaultAndroidInput (Application activity, Context context, Object view, AndroidApplicationConfiguration config) {
		...
		for (int i = 0; i < realId.length; i++)
			realId[i] = -1;
		handle = new Handler();
		this.app = new WeakReference<Application>(activity);
	...
	public void getTextInput(final TextInputListener listener, final String title, final String text, final String hint, final OnscreenKeyboardType keyboardType) {
		handle.post(new Runnable() {
			public void run () {
				...
				if (DefaultAndroidInput.this.context.get() == null) {
					return ;
				}
				context = DefaultAndroidInput.this.context.get();
	...
	public void setOnscreenKeyboardVisible (final boolean visible, final OnscreenKeyboardType type) {
		handle.post(new Runnable() {
			public void run () {
				...
				if (visible) {
					Application app = null;
					if (DefaultAndroidInput.this.app.get() == null) {
						return ;
					}
					app = DefaultAndroidInput.this.app.get();
